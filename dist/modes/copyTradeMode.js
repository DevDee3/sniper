...
export function startCopyTradeMode(connection, executor, riskManager, exitMonitor) {
        ...
        subscribeWalletLogs(connection, target, async (logs) => {
            try {
                ...
                if (event.side === "sell") {
                    ...
                    if (exited)
                        logger.trade(`Target sold ${event.mint} — mirrored exit`);
                    return;
                }
                ...
                const blockReason = riskManager.canOpenPosition(event.mint, decision.sizeSol);
                if (blockReason) {
