...
export function classifyLogs(logLines) {
    const joined = logLines.join("\n");
    if (joined.includes(LOG_MARKERS.CREATE))
        return "create";
    if (joined.includes(LOG_MARKERS.BUY) || joined.includes(LOG_MARKERS.SELL))
        return "trade";
    return null;
}
async function fetchParsedTx(connection, signature) {
    ...
    return throttledRpcCall(() => connection.getParsedTransaction(signature, {
 ...
 */
export async function parseCreateEvent(connection, signature, blockTime) {
 ...
 */
export async function parseTradeEvent(connection, signature, blockTime, traderHint) {
    const tx = await fetchParsedTx(connection, signature);
    if (!tx || !tx.meta)
        return null;
    ...
    const traderTokenPost = post.find((b) => b.owner === trader);
    const traderTokenPre = pre.find((b) => b.owner === trader && b.mint === traderTokenPost?.mint);
    if (!traderTokenPost)
        return null;
    const mint = traderTokenPost.mint;
    const preAmt = traderTokenPre?.uiTokenAmount.uiAmount ?? 0;
    const postAmt = traderTokenPost.uiTokenAmount.uiAmount ?? 0;
